...
<body style="font-family: Arial, sans-serif; background-color: #f4f4f9; margin: 0; padding: 0; display: flex; justify-content: center; align-items: center; min-height: 100vh;">
    <div style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); width: 100%; max-width: 500px;">
        <h1 style="text-align: center; color: #333; margin-bottom: 20px;">Editar Produto</h1>
        ...
        <form action="{{ route('produtos.update', $produto->id) }}" method="POST" style="display: flex; flex-direction: column; gap: 15px;">
            @csrf
            @method('PUT')
            <div>
                <label for="nome" style="display: block; margin-bottom: 5px; color: #333;">Nome</label>
                <input type="text" name="nome" id="nome" value="{{ old('nome', $produto->nome) }}" style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; font-size: 16px;" required>
            </div>

            <div>
                <label for="preco" style="display: block; margin-bottom: 5px; color: #333;">Preço</label>
                <input type="number" name="preco" id="preco" value="{{ old('preco', $produto->preco) }}" step="0.01" style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; font-size: 16px;" required>
            </div>

            <div>
                <label for="descricao" style="display: block; margin-bottom: 5px; color: #333;">Descrição</label>
                <textarea name="descricao" id="descricao" style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; font-size: 16px; resize: vertical; min-height: 100px;">{{ old('descricao', $produto->descricao) }}</textarea>
            </div>
